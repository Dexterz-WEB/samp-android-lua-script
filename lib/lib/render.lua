...
function getRenderRect(render)
	...
	for line in render.rtext:gmatch('[^\n]+') do
		...
		if height > 0 then
			...
			height = 0.001 -- êîñòûëü äëÿ ïðîïóñêà ïåðâîé ñòðî÷êè ïðè ïîäñ÷åòå îáùåé âûñîòû
		end
	end
	return {
		['top'] = top,
...
-- Substitua todo o trecho problemático do render.lua (linhas 187-195) por:

local drag_enabled = false
local current_drag_render = nil

-- Comando para ativar/desativar o modo de arrasto
sampRegisterChatCommand("dragmode", function()
    drag_enabled = not drag_enabled
    sampAddChatMessage(drag_enabled and "{00FF00}Modo arrasto ativado. Use /drag X Y para mover." or "{FF0000}Modo arrasto desativado.", -1)
end)

-- Comando para mover elementos
sampRegisterChatCommand("drag", function(param)
    if not drag_enabled or not current_drag_render then return end
    
    local x, y = param:match("(%S+)%s+(%S+)")
    x, y = tonumber(x), tonumber(y)
    
    if x and y then
        current_drag_render.pos({x = x, y = y})
        current_drag_render.ondragprocess()
        sampAddChatMessage("{FFFF00}Posição atualizada: "..x..", "..y, -1)
    else
        sampAddChatMessage("{FF0000}Uso: /drag X Y", -1)
    end
end)

-- Comando para selecionar elemento para arrastar
sampRegisterChatCommand("selectdrag", function()
    if not drag_enabled then
        sampAddChatMessage("{FF0000}Ative primeiro o modo arrasto com /dragmode", -1)
        return
    end
    
    -- Aqui você pode implementar a lógica para selecionar qual elemento arrastar
    -- Por exemplo, usar uma lista de elementos disponíveis
    current_drag_render = renders[1] -- Exemplo: seleciona o primeiro render
    if current_drag_render then
        sampAddChatMessage("{00FF00}Elemento selecionado para arrasto. Use /drag X Y", -1)
    else
        sampAddChatMessage("{FF0000}Nenhum elemento disponível para arrasto", -1)
    end
end)
...
lua_thread.create(function()
    while true do
        wait(1000)
        -- Mantém a verificação periódica dos renders visíveis
