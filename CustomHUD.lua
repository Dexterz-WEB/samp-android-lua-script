...
-- ============================================================================
-- SINGLE imgui.OnFrame (EVERYTHING in one frame - same as RadialMenu.lua)
-- ============================================================================
imgui.OnFrame(function() return true end, function()
    ...
    -- ========================================================================
    -- CONFIG WINDOW (inside same OnFrame)
    -- ========================================================================
    if showConfig[0] then
        imgui.SetNextWindowPos(imgui.ImVec2(50, 50), imgui.Cond.FirstUseEver)
        imgui.SetNextWindowSize(imgui.ImVec2(400, 320))
        ...
        imgui.End()
        return  -- Don't render HUD while config is open
    end

    -- ========================================================================
    -- HUD RENDERING (only when config is closed)
    -- ========================================================================
    ...
    local hpFill = math.max(0, math.min(hudData.health / 100, 1.0)) * 200
    dl:AddText(imgui.ImVec2(hpX, hpY - 1), 0xCC50AF4C, "HP")
    dl:AddRectFilled(imgui.ImVec2(hpX + 25, hpY), imgui.ImVec2(hpX + 225, hpY + 16), 0xFF1A2E1A, 3)
    if hpFill > 0 then
        dl:AddRectFilled(imgui.ImVec2(hpX + 25, hpY), imgui.ImVec2(hpX + 25 + hpFill, hpY + 16), 0xFF50AF4C, 3)
    end
    dl:AddText(imgui.ImVec2(hpX + 230, hpY + 1), 0xFF50AF4C, tostring(math.floor(hudData.health)))

    -- Armor Bar
    if hudData.armor > 0 then
        local arX, arY = leftX + 12, leftY + 62
        local arFill = math.max(0, math.min(hudData.armor / 100, 1.0)) * 200
        dl:AddText(imgui.ImVec2(arX, arY - 1), 0xCCF39621, "AR")
        dl:AddRectFilled(imgui.ImVec2(arX + 25, arY), imgui.ImVec2(arX + 225, arY + 16), 0xFF0D2A3E, 3)
        if arFill > 0 then
            dl:AddRectFilled(imgui.ImVec2(arX + 25, arY), imgui.ImVec2(arX + 25 + arFill, arY + 16), 0xFFF39621, 3)
        end
        dl:AddText(imgui.ImVec2(arX + 230, arY + 1), 0xFFF39621, tostring(math.floor(hudData.armor)))
    end
    ...
    local ammoText = hudData.weaponId == 0 and wepName or (wepName .. "  " .. hudData.ammoClip .. " / " .. hudData.ammoTotal)
