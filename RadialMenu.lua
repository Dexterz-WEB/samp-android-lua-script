...
-- ============================================================================
-- IMGUI ONFRAME (TOP LEVEL - MonetLoader requires this outside main())
-- ============================================================================
imgui.OnFrame(function() return true end, function(self)
    local anyMenuOpen = showRadialMenu[0] or showConfigWindow[0] or showCatRadial[0] or showAnimRadial[0] or showContextVehRadial[0]
    self.HideCursor = not anyMenuOpen

    local sw, sh = getScreenResolution()
    local cx, cy = sw / 2, sh / 2

    -- Fullscreen overlay window for draw_list
    ...
    imgui.Begin('##RadialOverlayBG', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.NoInputs)
    ...
    if menuScale > 0.01 then
        ...
        local pie_draw_list = imgui.GetWindowDrawList()
        ...
        if showRadialMenu[0] then
            ...
            local hoveredSector, centerHovered = drawPieMenu(pie_draw_list, cx, cy, menuItems, menuScale, "[MAIN]", {1, 1, 0})
        ...
        if showContextVehRadial[0] then
            ...
            local hoveredSector, centerHovered = drawPieMenu(pie_draw_list, cx, cy, menuItems, menuScale, titleLabel, {0.53, 0.86, 1.0})
        ...
        if showCatRadial[0] then
            ...
            local hoveredSector, centerHovered = drawPieMenu(pie_draw_list, cx, cy, menuItems, menuScale, "[ANIM]", {0, 1, 1})
        ...
        if showAnimRadial[0] then
            ...
            local hoveredSector, centerHovered = drawPieMenu(pie_draw_list, cx, cy, menuItems, menuScale, titleText, {0, 1, 1})
...
function main()
    ...
    sampAddChatMessage("{00FFFF}[Radial Menu] {FFFFFF}v3.0.1 loaded successfully!", -1)
